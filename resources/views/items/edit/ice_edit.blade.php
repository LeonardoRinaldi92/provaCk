...
<div class="container mt-3">
    <h2>Modifica {{$ice->name}}</h2>
    <form method="POST" action="{{ route('items.ices.update', ['ices' => $ice]) }}" id="form" enctype="multipart/form-data">
        @csrf
        @method('PUT') <!-- Aggiungi il metodo PUT per l'aggiornamento -->
        <div class="form-group">
            <label for="name">Nome:</label>
            <input type="text" class="form-control w-25 @error('name') is-invalid @enderror" id="name" name="name" placeholder="Inserisci nome" minlength="3" maxlength="50" value="{{ old('name', $ice->name) }}" required pattern="^[A-Za-z0-9À-Åà-åÈ-Ëè-ëÌ-Ïì-ïÒ-Öò-öÙ-Üù-üéèà&\-\s]+$">
            @error('name')
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" id="description" name="description" placeholder="Inserisci una descrizione">{{ old('description', $ice->description) }}</textarea>
        </div>
        <div class="form-group">
            ...
            @if ($ice->image)
            <div class="mt-2">
                <img src="{{ asset('storage/'. $ice->image) }}" alt="Immagine {{ $ice->name }}" style="max-height: 200px" id="image-preview">
            </div>
            @endif
        </div>
