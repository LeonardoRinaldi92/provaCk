...
<div class="container mt-3">
    <h2>Modifica {{$equipement->name}}</h2>
    <form method="POST" action="{{ route('items.equipements.update', ['equipements' => $equipement]) }}" id="form" enctype="multipart/form-data">
        @csrf
        @method('PUT') <!-- Aggiungi il metodo PUT per l'aggiornamento -->
        <div class="form-group">
            <label for="name">Nome:</label>
            <input type="text" class="form-control w-25 @error('name') is-invalid @enderror" id="name" name="name" placeholder="Inserisci nome" minlength="3" maxlength="50" value="{{ old('name', $equipement->name) }}" required pattern="^[A-Za-z0-9À-Åà-åÈ-Ëè-ëÌ-Ïì-ïÒ-Öò-öÙ-Üù-üéèà&\-\s]+$">
            @error('name')
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" id="description" name="description" placeholder="Inserisci una descrizione">{{ old('description', $equipement->description) }}</textarea>
        </div>
        <div class="form-group">
            ...
            @if ($equipement->image)
            <div class="mt-2">
                <img src="{{ asset('storage/'. $equipement->image) }}" alt="Immagine {{ $equipement->name }}" style="max-height: 200px" id="image-preview">
            </div>
            @endif
        </div>
...
if(nomeOriginale !== value){
    if (value.length > 2) {
        return fetch("{{ route('check.Equipement') }}", {
            method: 'POST',
            headers: {
