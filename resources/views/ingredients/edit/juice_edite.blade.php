...
<div class="container mt-3">
    <h2>Modifica {{$juice->name}}</h2>
    <form method="POST" action="{{ route('ingredients.juices.update', ['juices' => $juice]) }}" id="form">
        @csrf
        @method('PUT') <!-- Aggiungi il metodo PUT per l'aggiornamento -->
        <div class="form-group">
            <label for="name">Nome:</label>
            <input type="text" class="form-control w-25 @error('name') is-invalid @enderror" id="name" name="name" placeholder="Inserisci nome" minlength="3" maxlength="50" value="{{ old('name', $juice->name) }}" required pattern="^[A-Za-z0-9À-Åà-åÈ-Ëè-ëÌ-Ïì-ïÒ-Öò-öÙ-Üù-üéèà&\-\s]+$">
            @error('name')
    ...
    <form method="POST" action="{{ route('ingredients.juices.destroy', ['juices' => $juice]) }}" id="deleteForm">
        @csrf
...
if(nomeOriginale !== value){
    if (value.length > 2) {
        return fetch("{{ route('check.Juices') }}", {
            method: 'POST',e
            headers: {
