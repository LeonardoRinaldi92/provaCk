...
<div class="container mt-3">
    <h2>Modifica {{$fruit->name}}</h2>
    <form method="POST" action="{{ route('ingredients.fruits.update', ['fruits' => $fruit]) }}" id="form">
        @csrf
        @method('PUT') <!-- Aggiungi il metodo PUT per l'aggiornamento -->
        <div class="form-group">
            <label for="name">Nome:</label>
            <input type="text" class="form-control w-25 @error('name') is-invalid @enderror" id="name" name="name" placeholder="Inserisci nome" minlength="3" maxlength="50" value="{{ old('name', $fruit->name) }}" required pattern="^[A-Za-z0-9À-Åà-åÈ-Ëè-ëÌ-Ïì-ïÒ-Öò-öÙ-Üù-üéèà&\-\s]+$">
            @error('name')
    ...
    <form method="POST" action="{{ route('ingredients.fruits.destroy', ['fruits' => $fruit]) }}" id="deleteForm">
        @csrf
